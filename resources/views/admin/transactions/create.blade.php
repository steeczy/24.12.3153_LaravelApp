...
@section('title', 'Tambah Transaksi Baru - Admin')
@section('page_title', 'Tambah Transaksi Baru')
@section('page_subtitle', 'Masukkan data transaksi manual.')
        ...
        <form action="{{ route('admin.transactions.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="total_price" value="{{ old('total_price', 0) }}"
                        class="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium"
                        required min="0">
                    @error('total_price')
                        <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
            ...
            <div>
                ...
                <input type="text" name="snap_token" value="{{ old('snap_token') }}"
                    class="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium">
                @error('snap_token')
                    <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                @enderror
