...
@section('page_subtitle', 'Atur daftar partner dan logo di sini.')

@section('content')
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
        <form action="{{ route('admin.partners.index') }}" method="GET" class="flex flex-wrap gap-3 items-center">
            <div class="min-w-[220px]">
                <label for="search" class="sr-only">Cari Partner</label>
            ...
            <button type="submit"
                class="px-5 py-3 bg-slate-900 text-white rounded-2xl font-bold shadow-sm hover:bg-slate-800 active:scale-95 transition">
                Cari
            </button>
            @if (!empty($search))
                <a href="{{ route('admin.partners.index') }}"
                    class="px-5 py-3 bg-slate-100 text-slate-700 rounded-2xl font-bold hover:bg-slate-200 transition">
                    Reset
                </a>
            @endif
        </form>
                ...
                <tbody class="divide-y border-t">
                    @forelse($partners as $index => $partner)
                        <tr class="hover:bg-slate-50/50 transition">
                            ...
                            <td class="px-8 py-6">
                                <img src="{{ $partner->logo_url }}" class="w-16 h-16 rounded-2xl object-cover shadow-sm"
                            ...
                            <td class="px-8 py-6">
                                <a href="{{ $partner->logo_url }}" target="_blank" rel="noopener"
                                    class="text-sm text-indigo-600 font-bold hover:underline">{{ $partner->logo_url }}</a>
                            </td>
                            <td class="px-8 py-6">
