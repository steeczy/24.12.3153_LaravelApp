...
@section('content')
    ...
    <section id="events" class="max-w-7xl mx-auto px-6 py-20">
        ...
        <div class="mb-8 flex gap-4 justify-center flex-wrap">
            ...
            <a href="/"
                class="px-4 py-2 rounded transition {{ request('category') ? 'bg-indigo-100 hover:bg-indigo-200 text-indigo-700' : 'bg-indigo-600 text-white hover:bg-indigo-700' }}">Semua
                Kategori</a>

            <!-- Melakukan iterasi nama Tab Kategori dinamis saat jumlah data bertambah  -->
            @foreach ($categories as $cat)
                <a href="/?category={{ $cat->slug }}"
                    class="px-4 py-2 rounded shadow-sm transition {{ request('category') === $cat->slug ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-indigo-100 hover:bg-indigo-200 text-indigo-700' }}">
                    {{ $cat->name }}
                </a>
            @endforeach
        </div>
    ...
    <!-- Partners Section -->
    <section id="partners" class="max-w-7xl mx-auto px-6 pb-24">
        <div class="flex flex-wrap items-end justify-between gap-4 mb-10">
            <div>
                <h2 class="text-3xl font-extrabold mb-2">Partner Kami</h2>
                <p class="text-slate-500 font-medium">Mitra yang mendukung AmikomEventHub.</p>
            </div>
            <span class="text-xs uppercase tracking-[0.2em] text-slate-400 font-bold">Trusted By</span>
        </div>

        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
            @forelse ($partners as $partner)
                <div class="bg-white border border-slate-100 rounded-2xl p-4 flex flex-col items-center justify-center gap-3 shadow-sm">
            ...
            @empty
                <div class="col-span-full bg-white border border-slate-100 rounded-2xl p-8 text-center text-slate-500">
